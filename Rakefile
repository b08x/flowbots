...
namespace :rdoc do
  desc "Generate PDF from RDoc HTML output"
  task :pdf => :rdoc do
    begin
      require 'pdfkit'
      require 'fileutils'

      puts "Generating PDF from RDoc documentation..."
      ...
      # Path to the index.html file generated by RDoc
      input_file = File.join(APP_ROOT, 'doc', 'index.html')
      output_file = File.join(APP_ROOT, 'doc', 'flowbots_documentation.pdf')

      unless File.exist?(input_file)
        puts "Error: RDoc HTML files not found. Please run 'rake rdoc' first."
        exit 1
      end

      # Create PDF
      kit = PDFKit.new(File.read(input_file), {
        :protocol => 'http',
        :root_url => 'http://localhost'
      })

      # Add all CSS files
      Dir[File.join(APP_ROOT, 'doc', '**', '*.css')].each do |css_file|
      ...
      puts "Error: Required gem 'pdfkit' not found. Please install it with:"
      puts "gem install pdfkit"
      puts "You'll also need to install wkhtmltopdf:"
      puts "sudo apt-get install wkhtmltopdf  # for Ubuntu/Debian"
      puts "brew install wkhtmltopdf          # for macOS"
      exit 1
    rescue => e
  ...
  desc "Generate both HTML and PDF documentation"
  task :all => [:rdoc, :pdf]
end
